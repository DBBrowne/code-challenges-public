...
function testFn(fnToTest, testCase, showPass){
  ...
  if (Array.isArray(output)){
    assert = output.every(function (subResult, index) {
      return subResult === testCase[1][index]
    })
  } else if (typeof output === 'object' && output !== null) {
    assert = Object.keys(output).every(function (key) {
      return output[key] === testCase[1][key]
    })
  } else {
